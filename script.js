...
function clearPlayersList() {
    ...
    playersGrid.innerHTML = `
        <div style="text-align: center; color: var(--text-secondary); grid-column: 1 / -1; padding: 2rem;">
            ...
            <p>vortexsmp.live is currently offline</p>
...
function initializeContactForm() {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            ...
            const subject = encodeURIComponent(`VortexSMP Contact Form - Message from ${name}`);
            const body = encodeURIComponent(`From: ${name} (${email})\n\nMessage:\n${message}`);
            const mailtoLink = `mailto:admin@vortexsmp.live?subject=${subject}&body=${body}`;

            // Open email client
            window.open(mailtoLink);

            showNotification('Email client opened! Message will be sent to admin@vortexsmp.live', 'success');
            this.reset();
        });
    }
}
...
function openDiscord() {
    window.open('https://discord.gg/uutjmpNp', '_blank');
}
...
function initializeAdvancedFeatures() {
    ...
    const searchableContent = [
        { title: 'Server IP', content: 'vortexsmp.live', section: '#home' },
        ...
        addStaffForm.addEventListener('submit', function(e) {
            ...
            if (!usersDatabase[websiteUsername]) {
                usersDatabase[websiteUsername] = {
                    ...
                    email: `${websiteUsername}@vortexsmp.live`,
            ...
            staffDatabase[websiteUsername] = {
                ...
                minecraftUsername: minecraftUsername,
                rank: rank.toLowerCase(),
                bio: bio || `${rank.charAt(0).toUpperCase() + rank.slice(1)} on VortexSMP`,
                dateAdded: new Date().toISOString()
            };
...
function drawOverworldMap(ctx, canvas) {
    ...
    ctx.fillText(`🌍 VortexSMP World (Seed: ${mapSeed})`, 10, 25);
...
function drawNetherMap(ctx, canvas) {
    ...
    ctx.fillStyle = '#D4AF37';
    ctx.font = '16px Arial';
    ctx.fillText('The Nether - VortexSMP', 20, 30);
}
...
function drawEndMap(ctx, canvas) {
    ...
    ctx.fillStyle = '#D4AF37';
    ctx.font = '16px Arial';
    ctx.fillText('The End - VortexSMP', 20, 30);
}
